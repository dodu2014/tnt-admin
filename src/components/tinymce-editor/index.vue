...
const props = defineProps({
  ...
  uploadHandlerUrl: {
    type: String,
    default: `/Base/UpFiles/upload?waterMark=${encodeURIComponent('TNT-Admin')}&waterMarkPosition=bottomright`,
  },
  theme: {
    type: String,
    default: 'auto',
...
const editorInitOptions = computed(() => ({
  ...
  language: 'zh-Hans', // 语言
  content_style: 'html,body { height: 100%; margin: 0; } body { box-sizing: border-box; overflow-y: auto; padding: 1em; } img { max-width: 100% }', // 自定义 CSS 样式应用与内容
  ...
  images_upload_handler: (blobInfo, progress: any) =>
    new Promise((resolve, reject) => {
      ...
      request
        .post({ url: props.uploadHandlerUrl, data: formData })
        .then(res => {
          console.log('tinymce editor upload result:', res)
          if (res.url && res.absUrl) {
            resolve(res.absUrl) // 上传成功，在成功函数里填入图片路径
          } else {
            reject(new Error(`HTTP Error: 上传失败`))
          }
        })
        .catch(e => {
          console.error(e)
  ...
  file_picker_callback: (callback, value, meta) => {
    ...
    input.addEventListener('change', _ => {
      ...
      request
        ...
        .then(res => {
          console.log('tinymce editor upload result:', res)
          callback(res.absUrl, { title: fileblob.name })
        })
        .catch(e => {
          console.error(e)
        })
